  ...
  while (not q.empty()) {
    ...
    q.pop();
    {
      ...
      if (safe(top) and d[at(top)] == -1) {
        if (map[top.first][top.second] == 't') return d[at(c)] + 1;
        q.push(top);
        d[at(top)] = d[at(c)] + 1;
      }
    }
    {
      ...
      if (safe(bot) and d[at(bot)] == -1) {
        if (map[bot.first][bot.second] == 't') return d[at(c)] + 1;
        q.push(bot);
        d[at(bot)] = d[at(c)] + 1;
      }
    }
    {
      ...
      if (safe(left) and d[at(left)] == -1) {
        if (map[left.first][left.second] == 't') return d[at(c)] + 1;
        q.push(left);
        d[at(left)] = d[at(c)] + 1;
      }
    }
    {
      ...
      if (safe(right) and d[at(right)] == -1) {
        if (map[right.first][right.second] == 't') return d[at(c)] + 1;
        q.push(right);
        d[at(right)] = d[at(c)] + 1;
      }
    }

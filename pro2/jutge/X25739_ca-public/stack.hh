...
public:
    ...
    bool empty() const { return ptopitem == NULL; }

    void push(T value) {
        T sum_below = 0;
        if (!empty()) sum_below = ptopitem->sum_below + ptopitem->value;

        Item *pnewitem = new Item();
        pnewitem->value = value;
        pnewitem->next = ptopitem;
        pnewitem->sum_below = sum_below;
        ptopitem = pnewitem;
        _size++;
    }
    ...
    T sum() const {
        if (empty()) return 0;
        return ptopitem->sum_below + ptopitem->value;
    }
    ...
    // Pre:  Sigui [a1,...,an] el contingut actual de la pila des del fons fins
    // al top. Post: Retorna a1+...+an. Descomenteu les següents dues linies i
    // implementeu el mètode: T sum() {
    // }
};

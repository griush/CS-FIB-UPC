...
class BinTree {
    ...
    //   - default destructor is good. Θ(n) where n is the number of nodes in
    //   the tree.
    ...
    */
    ...
    explicit BinTree(const T &x) { p = make_shared<Node>(x, nullptr, nullptr); }
    ...
    // writing operator <<
    /* Pre: true */
    /* Post: a representation of t is written on os deppending on current value
     * of attribute 'outputformat' */
    template <class U>
    friend std::ostream &operator<<(std::ostream &os, const BinTree<U> &t);

    // reading operator >>
    // Pre: 'is' contains a representation of a binary tree deppending on
    // current value of attribute 'inputformat'. Post: t is assigned the tree
    // value read from 'is'
    template <class U>
...
string BinTree<T>::value2string(const T &value) {
    ...
    for (int i = 0; i < int(s.size()); ++i)
        if (s[i] == ' ')
            dieInputOutputBinTree(
                "Error: white spaces are not allowed for strings describing "
                "values");
    return s;
}

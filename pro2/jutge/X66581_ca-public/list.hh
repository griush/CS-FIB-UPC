...
class List {
   private:
    ...
    void removeItems() {
        ...
        while (_size > 0) removeItem(iteminf.next);
    }

    void copyItems(List &l) {
        /* Pre: cert*/
        /* Post: copia els elements de la llista l al p.i. */
        for (Item *pitem = l.itemsup.prev; pitem != &l.iteminf;
             pitem = pitem->prev)
   ...
   public:
    ...
    // pre: l is immediatly before r
    // post: r have switched positions
    void swap(Item *l, Item *r) {
        Item *l_prev = l->prev;
        Item *r_next = r->next;

        if (l_prev) l_prev->next = r;
        if (r_next) r_next->prev = l;
    ...
    class iterator {
       ...
       public:
        iterator() { has_hook = false; }

        // Adapt this function so that moving beyond boundaries does not trigger
        // error, but leaves the iterator unchanged instead. Also, add the
        // necessary adaptations so that, when the iterator has an active hook,
        // instead of making the iterator point to next element (towards the end
        // of the list), the iterator keeps pointing to the same element, and
        // this element swaps its position with the next one (towards the end of
        // the list). In the event that there is no such a next element, nothing
        // changes. Preincrement
        iterator operator++()
           ...
           el resultat és el p.i. */
        {
            ...
            if (pitem->next != &(plist->itemsup)) plist->swap(pitem, pitem->next);
            return *this;
        }
        ...
        // error, but leaves the iterator unchanged instead. Also, add the
        // necessary adaptations so that, when the iterator has an active hook,
        // instead of making the iterator point to previous element (towards the
        // begin of the list), the iterator keeps pointing to the same element,
        // and this element swaps its position with previous one (towards the
        // begin of the list). In the event that there is no such a previous
        // element, nothing changes. Predecrement
        iterator operator--()
           ...
           el resultat és el p.i. */
        {
            ...
            if (pitem->prev != &(plist->iteminf)) plist->swap(pitem->prev, pitem);
            return *this;
        }
        ...
        // Pre: Iterator 'this' (the implicit parameter) does not have an active
        // hook,
        //      and it points to an element of a list.
        //      In particular, 'this' does not point to the end of a list.
        // Post: 'it' keeps pointing to the same element and has an active hook
        // to this element.
        void hook() { has_hook = true; }

        // Pre: 'this' has an active hook.
        // Post: 'this' does not have an active hook.
        void stopHook() { has_hook = false; }

        // Pre:
        // Post: Returns true iff 'this' has an active hook.
        bool hasActiveHook() const { return has_hook; }
    };
       ...
       resultat apunta a l'element posterior a l'eliminat */
    {
        ...
        if (it.pitem == &itemsup) {
            cout << "Error: erase with an iterator pointing to the end of the "
                    "list"
                 << endl;
            exit(1);
        }
...
ostream &operator<<(ostream &os, List<T> &l) {
    os << l._size;
    for (typename List<T>::Item *pitem = l.iteminf.next; pitem != &l.itemsup;
         pitem = pitem->next)
        cout << " " << pitem->value;
    return os;
}
